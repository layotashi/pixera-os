 ...
 *
 * TESS の identity は **39桁**（PERFORM の最小幅 480px に対称余白込みで収まる桁数）。整形はこの制約を尊重する。
 * トークン列を行（row）へまとめ、正準な空白・インデント・**39桁折り返し**で出し直す。
 * 字句エラー時はソースをそのまま返す（壊れたコードは整形しない＝安全）。
 *
 * 整形の規則:
 *   - 二項 `+ -` は前後 1 スペース、`* / % ^` は詰める（優先順位ベース）。`f(`/単項 `-` は詰め。
 *   - `{` で字下げ +1、`}` で −1（`}` は単独行）。`;`/改行は「1 文 1 行」へ畳む。空行は 1 行残す。
 *   - **(A) 39桁折り返し**: 39桁超の式を、低優先 `+ -` で改行し演算子を継続行頭へ出す
 ...
 *   - **(C) 代入整列**: 同深度で連続する `name = …` の `=` を揃える（39桁を超えない範囲でのみ）。
 *   - **(E) コメント正規化**: `//x` → `// x`（先頭 1 スペース）。本文・ブロックコメントは不変。
 *   - **(N) 数値正準化**: `0.5`→`.5`、`1.0`→`1`、末尾ゼロ除去（GLSL/tixy 慣習・39桁節約・意味不変）。
 *
...
const INDENT = "  ";
const WIDTH = 39; // TESS の桁数（折り返しの目標幅）
const TIGHT = new Set(["*", "/", "%", "^"]);
...
/**
 * 数値字面の正準化（GLSL/tixy 慣習＋39桁節約）。意味は不変（同じ値）。
...
/**
 * ノード列を 39桁で折り返して物理行配列にする（indent/prefix は 1 行目の先頭）。
 * 改行は paren の内側でのみ起き、文区切りにならない（深度0の式は括弧を補ってから折る）。
 * 候補（折らない案 / 既存の各 paren を折る案 / 深度0を括弧で包む A2 案）を出し、
 * 「39桁に収まる案を優先 → 同点なら最小行数」で選ぶ。これで WAVE 系は最短（既存 paren）、
...
// ── (C) 代入整列: 同深度連続グループの `=` を 39桁内でのみ揃える ───────────────
...
function renderAssignGroup(group) {
  ...
  // 揃えても 39桁を超えないときだけ整列（超えるなら素のまま＝折り返しに委ねる）。
