...
music/*.js          → FocusableWidget, ports.js, ui_helpers.js (音楽制作系ウィジェット)
...
ハードウェア音楽機材の操作子を 1-bit で再現した別カテゴリ。OS 標準ウィジェットとは
用途を分け、SYNTH / MIXER / SAMPLER / DAW など音を扱うアプリでのみ使う。ポート・基底
クラスは共通なので `initPorts()` を 1 度呼べば動作する。`music/index.js` から import する。

- `Fader` — 垂直フェーダー (コンソール・フェーダー)。幅固定・高さ可変で、必要な本数を
  横に並べてバンクを作る。操作系は水平 `Slider` と対称 (上下ドラッグ / Shift 微調整 /
  ホイール / ダブルクリックで既定値 / ↑↓ キー)。数値は表示せず、ラベルはアプリが上に描く。
