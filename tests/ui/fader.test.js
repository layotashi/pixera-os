...
describe("Fader — ドラッグの値マッピング (上=max, 下=min)", () => {
  // travelTop = y+5 = 5, travelBottom = y+h-6 = 38 (h=44)
  it("上端をつかむと max になる", () => {
    const f = new Fader(0, 0, 44, 0, 100, 50);
    press(f, 5);
  ...
  it("中央をつかむと中間値になる", () => {
    const f = new Fader(0, 0, 44, 0, 100, 0);
    press(f, Math.round((5 + 38) / 2)); // ≈ 22
...
describe("Fader — onChange / ホイール / ダブルクリック", () => {
  it("値が変わったら onChange が新値で呼ばれる", () => {
    let got = null;
    const f = new Fader(0, 0, 44, 0, 100, 50, (v) => (got = v));
    press(f, 5);
    expect(got).toBe(100);
  });
  ...
  it("同値なら onChange は呼ばれない", () => {
    let calls = 0;
    const f = new Fader(0, 0, 44, 0, 100, 100, () => calls++);
    press(f, 5); // すでに max
    expect(calls).toBe(0);
  });
  ...
  it("ダブルクリックでデフォルト値に戻る", () => {
    const f = new Fader(0, 0, 44, 0, 100, 30);
    press(f, 5); // 100 に変更
    expect(f.value).toBe(100);
...
describe("Fader — 非整数レンジ / 描画スモーク", () => {
  it("非整数レンジでは丸めない", () => {
    const f = new Fader(0, 0, 44, 0, 1.5, 0.5);
    expect(f._isInt).toBe(false);
    press(f, 22);
    expect(Number.isInteger(f.value)).toBe(false);
  });

  it("draw が例外を投げない (各値位置)", () => {
    const f = new Fader(2, 2, 44, 0, 100, 50);
    const cr = { x: 0, y: 0 };
